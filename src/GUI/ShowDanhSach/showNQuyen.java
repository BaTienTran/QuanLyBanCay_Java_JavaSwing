 ...
 * @author PC
 */
public class showNQuyen extends javax.swing.JDialog {

    /**
     * Creates new form showNCCap
     */
    NhomQuyenDTO nhomQuyenChon = null;
    ...
    public showNQuyen() {
        ...
        tblNhomQuyen.setDefaultEditor(Object.class, null); // set ko cho sửa dữ liệu trên table
        tblNhomQuyen.getColumnModel().getColumn(1).setPreferredWidth(180);
        tblNhomQuyen.setFocusable(false);
        tblNhomQuyen.setAutoCreateRowSorter(true);
        
        hienThiNhomQuyen(listNhomQuyen);
    }

    
    public void hienThiNhomQuyen(ArrayList<NhomQuyenDTO> listNhomQuyen){
        DefaultTableModel dtm = (DefaultTableModel) tblNhomQuyen.getModel();
        dtm.setRowCount(0);
        for(NhomQuyenDTO nhomQuyenDTO : listNhomQuyen){
            Object[] row = {
                nhomQuyenDTO.getMaNhomQuyen(),
    ...
    private void btnLuuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLuuActionPerformed
        ...
        if(tblNhomQuyen.getSelectedRow()!=-1){
            int row = tblNhomQuyen.getSelectedRow();
            TableModel model = tblNhomQuyen.getModel();
            int manq = Integer.parseInt(model.getValueAt(row, 0).toString());
            nhomQuyenChon = nhomQuyenBUS.getNhomQuyen(manq);
            dispose();
        } else {
